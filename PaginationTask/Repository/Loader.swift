...
//  BeerPhotoDemo
//
//  Created by Rajinder Singh on 21/02/23.
//
...
class Loader:NSObject{
    ...
    static var baseBackColor = UIColor(white: 1, alpha: 0.5)
    ...
    static func start(backColor: UIColor = baseBackColor, baseColor: UIColor = baseColor) {
        ...
        if spinner == nil, let window = windowScene?.windows.first {
...
class DisplayBanner {
    class func show(message:String?){
        let banner = Banner(title: message ?? ErrorMessages.somethingWentWrong, subtitle: "", image: nil, backgroundColor: .lightGray)
...
struct Alert {
    static let alertAppName = "Pagination"
...
struct ErrorMessages {
    ...
    static let Connect_To_Internet = "Please connect to internet"
